    ...
    <div className="w-full max-w-sm rounded-xl bg-muted text-muted-foreground shadow">
      <Card className="shadow-none">
        <CardHeader className="flex flex-col items-center">
          ...
          <p className="text-balance text-center text-sm text-muted-foreground">Welkom terug! Log in om verder te gaan.</p>
        </CardHeader>
        <CardContent className="flex flex-col gap-6">
          {props.errorType ? (
            <Alert variant="destructive">
              <LuCircleAlert className="size-4" />
              <AlertDescription>{formatError(props.errorType)}</AlertDescription>
            </Alert>
          ) : null}
